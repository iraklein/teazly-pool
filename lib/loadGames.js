...
function areOddsLocked() {
  ...
  // Get current week's Wednesday
  const dayOfWeek = pst.getDay(); // 0=Sunday, 1=Monday, ..., 6=Saturday
  const wednesday = new Date(pst);
  
  // Calculate days to Wednesday (3)
  const daysToWednesday = (3 - dayOfWeek + 7) % 7;
  if (daysToWednesday > 0) {
    // If we haven't reached Wednesday yet
    wednesday.setDate(pst.getDate() + daysToWednesday);
  } else if (dayOfWeek > 3) {
    // If we're past Wednesday, get next Wednesday
    wednesday.setDate(pst.getDate() + (7 - dayOfWeek + 3));
  }
  // If it's Wednesday (dayOfWeek === 3), use today
  
  // Set Wednesday 10am PT
  wednesday.setHours(10, 0, 0, 0);
  
  // Check if current time is Wednesday 10am PT or later this week
  return pst >= wednesday && dayOfWeek >= 3;
}
...
export async function hasLiveGames() {
  try {
    const { data: liveGames, error } = await supabase
      .from('games')
      .select('id')
      .in('status', ['in_progress', 'live', 'halftime', 'overtime'])
      .limit(1);
...
async function fetchAndMatchOdds() {
  try {
    ...
    if (areOddsLocked()) {
      console.log('🔒 Odds locked (after Wednesday 10am PT) - skipping odds update');
      return { preseason: 0, regular: 0, errors: 0, locked: true };
    }
