...
export async function loadNFLGames() {
  try {
    ...
    // Load preseason weeks 1-3
    for (let week = 1; week <= 3; week++) {
      console.log(`Loading preseason week ${week}...`);
      
      const response = await fetch(`https://site.api.espn.com/apis/site/v2/sports/football/nfl/scoreboard?seasontype=1&week=${week}`);
      const data = await response.json();
      
      console.log(`Fetched ${data.events?.length || 0} games from preseason week ${week}`);
      ...
      if (data.events) {
        ...
        for (const game of data.events) {
          const gameData = {
            ...
            week_number: week, // Use the week we're querying for
          ...
          if (error) {
            ...
            console.log(`Updated week ${week} game: ${gameData.away_team} @ ${gameData.home_team} (${gameData.home_score || 0}-${gameData.away_score || 0})`);
            totalGamesLoaded++;
          }
        }
      }
...
export async function updateLiveScores() {
  try {
    ...
    // Update scores for preseason weeks 1-3
    for (let week = 1; week <= 3; week++) {
      console.log(`Checking scores for preseason week ${week}...`);
        ...
        try {
          ...
          if (gameUpdate.home_score !== null || gameUpdate.away_score !== null || gameUpdate.status) {
            ...
            if (error) {
              ...
              console.log(`Updated week ${week} scores: ${awayTeam.team.abbreviation} ${gameUpdate.away_score} - ${homeTeam.team.abbreviation} ${gameUpdate.home_score} (${gameUpdate.status})`);
              updated++;
            }
          }
        } catch (gameError) {
